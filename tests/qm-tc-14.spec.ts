...
class AppShell {
  ...
  private userMenuButton(): Locator {
    return this.page
      ...
      .or(this.page.getByRole('link', { name: /account|profile|user/i }));
  }
  ...
  async logout(): Promise<void> {
    // Try via user menu first.
    const userMenu = this.userMenuButton();
    ...
    // Fallback: direct navigation to common logout endpoints.
    await this.page.goto('/logout').catch(() => undefined);
  }
}
...
test.describe('QM-TC-14 Products visible for any authenticated role (TEST client)', { tag: '@new' }, () => {
  test("'Test Product' is visible for TESTER and ADMIN users", async ({ page }) => {
    // Arrange
    const testerUsername = process.env.TEST_USERNAME ?? process.env.APP_USERNAME;
    const testerPassword = process.env.TEST_PASSWORD ?? process.env.APP_PASSWORD;

    const adminUsername =
      process.env.ADMIN_USERNAME ?? process.env.TEST_USERNAME ?? process.env.APP_USERNAME;
    const adminPassword =
      process.env.ADMIN_PASSWORD ?? process.env.TEST_PASSWORD ?? process.env.APP_PASSWORD;

    if (!testerUsername || !testerPassword) {
      throw new Error('Missing TESTER credentials. Set TEST_USERNAME/TEST_PASSWORD (or APP_USERNAME/APP_PASSWORD).');
    }
    if (!adminUsername || !adminPassword) {
      throw new Error(
        'Missing ADMIN credentials. Set ADMIN_USERNAME/ADMIN_PASSWORD (or provide TEST_USERNAME/TEST_PASSWORD or APP_USERNAME/APP_PASSWORD as a fallback).',
      );
    }
    ...
    await loginPage.login(testerUsername, testerPassword);
    await shell.selectClient('TEST');
    await shell.openProducts();
    await catalog.assertProductVisible('Test Product');

    // Act (logout)
    await shell.logout();

    // Assert we are back on login page (or at least login form is visible)
    await expect(page.getByLabel(/user(name)?|email/i).first()).toBeVisible({ timeout: 30_000 });
